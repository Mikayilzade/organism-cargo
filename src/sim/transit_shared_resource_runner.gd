...
func simulate(committed_run: Dictionary, total_ticks: int, simulation_defs: Dictionary = {}) -> Dictionary:
	...
	for tick: int in range(1, total_ticks + 1):
		...
		if thermal_enabled:
			var propagated: Dictionary = thermal_kernel.propagate_heat(generated_environment.get("heat", {}), cell_order, thermal_rules)
		...
		if contamination_enabled:
			var contamination_result: Dictionary = contamination_kernel.propagate_phase_d(
				generated_environment.get("contamination", {}), cell_order, contamination_rules
			)
...
func _prepare_t07_definitions(simulation_defs: Dictionary) -> Dictionary:
	...
	return {
		"ok": true,
		"error": "",
		"producers": producers.duplicate(true),
		"consumers": consumers.duplicate(true),
	}

func _prepare_shared_satiety_runtime(
		organisms: Array,
		simulation_defs: Dictionary,
		t06_definitions: Array,
		t07_consumers: Array
) -> Dictionary:
	...
	if not organism_definitions_value is Dictionary:
		return _failure("missing_organism_definitions_for_satiety")
	var organism_definitions: Dictionary = organism_definitions_value
	var satiety_max_by_id: Dictionary = {}
	for definition_batch: Variant in [t06_definitions, t07_consumers]:
		var definitions: Array = definition_batch
		for raw_definition: Variant in definitions:
			if not raw_definition is Dictionary:
				return _failure("invalid_satiety_definition")
			var definition: Dictionary = raw_definition
			var instance_id: String = String(definition.get("instance_id", ""))
			var satiety_max: int = int(definition.get("satiety_max", 0))
			if instance_id.is_empty() or satiety_max <= 0:
				return _failure("invalid_satiety_definition")
			if satiety_max_by_id.has(instance_id) and int(satiety_max_by_id[instance_id]) != satiety_max:
				return _failure("conflicting_satiety_max:%s" % instance_id)
			satiety_max_by_id[instance_id] = satiety_max

	var initial_satiety_by_id: Dictionary = {}
	for raw_id: Variant in satiety_max_by_id.keys():
		...
		if initial_satiety < 0 or initial_satiety > int(satiety_max_by_id[instance_id]):
			return _failure("invalid_initial_satiety:%s" % instance_id)
		initial_satiety_by_id[instance_id] = initial_satiety

	var next: Array = organisms.duplicate(true)
	var matched: Dictionary = {}
	for index: int in range(next.size()):
		if not next[index] is Dictionary:
			return _failure("invalid_organism_runtime")
		var runtime: Dictionary = next[index]
		var instance_id: String = String(runtime.get("instance_id", ""))
		if initial_satiety_by_id.has(instance_id):
			runtime["satiety"] = int(initial_satiety_by_id[instance_id])
			next[index] = runtime
			matched[instance_id] = true
	if matched.size() != initial_satiety_by_id.size():
		return _failure("satiety_runtime_identity_mismatch")
	return {"ok": true, "error": "", "organisms": next}

func _compose_shared_satiety_phase_f(
		tick: int,
		pre_f_organisms: Array,
		t06_events: Array,
		t07_events: Array,
		t06_definitions: Array,
		t07_consumers: Array
) -> Dictionary:
	var satiety_max_result: Dictionary = _satiety_max_by_id(t06_definitions, t07_consumers)
	if not bool(satiety_max_result.get("ok", false)):
		return satiety_max_result
	var satiety_max_by_id: Dictionary = satiety_max_result["satiety_max_by_id"]
	var requested_delta_by_id: Dictionary = {}
	var parent_ids_by_id: Dictionary = {}
	var contributor_traits_by_id: Dictionary = {}

	for event_batch: Variant in [t06_events, t07_events]:
		var events: Array = event_batch
		for raw_event: Variant in events:
			if not raw_event is Dictionary:
				return _failure("invalid_shared_satiety_event")
			var event: Dictionary = raw_event
			if String(event.get("phase", "")) != "F":
				continue
			var trait_id: String = String(event.get("trait_id", ""))
			if trait_id != "T06" and trait_id != "T07":
				continue
			var instance_id: String = String(event.get("instance_id", event.get("consumer_id", "")))
			if instance_id.is_empty() or not satiety_max_by_id.has(instance_id):
				return _failure("invalid_shared_satiety_target")
			var requested_delta: int = int(event.get("satiety_delta", 0))
			if requested_delta < 0:
				return _failure("negative_shared_satiety_delta:%s" % instance_id)
			requested_delta_by_id[instance_id] = int(requested_delta_by_id.get(instance_id, 0)) + requested_delta
			var parents: PackedStringArray = _string_array(event.get("parent_event_ids", PackedStringArray()))
			var existing_parents: PackedStringArray = _string_array(parent_ids_by_id.get(instance_id, PackedStringArray()))
			for parent_id: String in parents:
				if parent_id not in existing_parents:
					existing_parents.append(parent_id)
			existing_parents.sort()
			parent_ids_by_id[instance_id] = existing_parents
			var traits: PackedStringArray = _string_array(contributor_traits_by_id.get(instance_id, PackedStringArray()))
			if trait_id not in traits:
				traits.append(trait_id)
			traits.sort()
			contributor_traits_by_id[instance_id] = traits

	var next: Array = pre_f_organisms.duplicate(true)
	var index_by_id: Dictionary = {}
	for index: int in range(next.size()):
		if not next[index] is Dictionary:
			return _failure("invalid_pre_f_organism_runtime")
		var runtime: Dictionary = next[index]
		var instance_id: String = String(runtime.get("instance_id", ""))
		if instance_id.is_empty() or index_by_id.has(instance_id):
			return _failure("invalid_pre_f_organism_identity")
		index_by_id[instance_id] = index

	var ids: PackedStringArray = PackedStringArray()
	for raw_id: Variant in requested_delta_by_id.keys():
		ids.append(String(raw_id))
	ids.sort()
	var events: Array = []
	for instance_id: String in ids:
		if not index_by_id.has(instance_id):
			return _failure("missing_shared_satiety_runtime:%s" % instance_id)
		var runtime_index: int = int(index_by_id[instance_id])
		var runtime: Dictionary = next[runtime_index]
		if not runtime.has("satiety"):
			return _failure("missing_satiety:%s" % instance_id)
		var before: int = int(runtime["satiety"])
		var satiety_max: int = int(satiety_max_by_id[instance_id])
		if before < 0 or before > satiety_max:
			return _failure("invalid_satiety:%s" % instance_id)
		var requested_delta: int = int(requested_delta_by_id[instance_id])
		var after: int = mini(satiety_max, before + requested_delta)
		runtime["satiety"] = after
		next[runtime_index] = runtime
		events.append({
			"event_id": "t%04d:F:SATIETY:%s" % [tick, instance_id],
			"tick": tick,
			"phase": "F",
			"kind": "SHARED_SATIETY_COMMIT",
			"instance_id": instance_id,
			"satiety_before": before,
			"satiety_requested_delta": requested_delta,
			"satiety_applied_delta": after - before,
			"satiety_after": after,
			"satiety_max": satiety_max,
			"contributor_traits": _string_array(contributor_traits_by_id.get(instance_id, PackedStringArray())),
			"parent_event_ids": _string_array(parent_ids_by_id.get(instance_id, PackedStringArray())),
		})
	return {"ok": true, "error": "", "organisms": next, "events": events}

func _satiety_max_by_id(t06_definitions: Array, t07_consumers: Array) -> Dictionary:
	...
	for definition_batch: Variant in [t06_definitions, t07_consumers]:
		...
		for raw_definition: Variant in definitions:
			...
			if instance_id.is_empty() or satiety_max <= 0:
				return _failure("invalid_satiety_definition")
			if result.has(instance_id) and int(result[instance_id]) != satiety_max:
				return _failure("conflicting_satiety_max:%s" % instance_id)
			result[instance_id] = satiety_max
	return {"ok": true, "error": "", "satiety_max_by_id": result}
...
func _serialize_t07_events(events: Array) -> String:
	var encoded: PackedStringArray = PackedStringArray()
	for raw_event: Variant in events:
		if raw_event is Dictionary:
			var event: Dictionary = raw_event
			encoded.append("%s:%s:%s:%s:%s:%d:%d:%d" % [
...
func _serialize_shared_satiety_events(events: Array) -> String:
	var encoded: PackedStringArray = PackedStringArray()
	for raw_event: Variant in events:
		if raw_event is Dictionary:
			var event: Dictionary = raw_event
			var parents: PackedStringArray = _string_array(event.get("parent_event_ids", PackedStringArray()))
			var traits: PackedStringArray = _string_array(event.get("contributor_traits", PackedStringArray()))
			encoded.append("%s:%s:%d:%d:%d:%d:%s:%s" % [
				String(event.get("event_id", "")),
				String(event.get("instance_id", "")),
				int(event.get("satiety_before", 0)),
				int(event.get("satiety_requested_delta", 0)),
				int(event.get("satiety_applied_delta", 0)),
				int(event.get("satiety_after", 0)),
				",".join(traits),
				",".join(parents),
			])
