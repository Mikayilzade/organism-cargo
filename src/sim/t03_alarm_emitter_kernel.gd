class_name T03AlarmEmitterKernel
extends RefCounted
...
func apply_phase_c(
	...
	if tick <= 0:
		return _failure("invalid_t03_tick")
	...
	for raw_organism: Variant in organisms:
		if not raw_organism is Dictionary:
			return _failure("invalid_t03_organism_runtime")
		...
		if instance_id.is_empty() or runtime_by_id.has(instance_id):
			return _failure("invalid_t03_organism_identity")
		runtime_by_id[instance_id] = organism

	var next_field: Dictionary = stress_field_by_cell.duplicate(true)
	var events: Array = []
	...
	for raw_definition: Variant in ordered_definitions:
		...
		if not runtime_by_id.has(instance_id):
			return _failure("missing_t03_organism_runtime:%s" % instance_id)
		...
		if not bool(eligibility["active"]):
			continue
		var occupied_result: Dictionary = _occupied_cells(organism, stress_field_by_cell)
		...
		for cell_key: String in occupied_cells:
			var before: int = int(next_field.get(cell_key, 0))
			if before < 0:
				return _failure("invalid_t03_stress_field_value:%s" % cell_key)
			var after: int = before + output_amount
			next_field[cell_key] = after
			events.append({
				"event_id": "t%04d:C:T03:%s:%s" % [tick, instance_id, cell_key],
				"tick": tick,
				"phase": "C",
				"kind": "T03_STRESS_FIELD_SOURCE",
				"trait_id": "T03",
				"instance_id": instance_id,
				"cell_key": cell_key,
				"stress_field_delta": output_amount,
				"stress_field_before": before,
				"stress_field_after": after,
	...
	return {
		"ok": true,
		"error": "",
		"stress_field_by_cell": next_field,
		"events": events,
...
func _validated_definitions(definitions: Array) -> Dictionary:
	...
	for raw_definition: Variant in definitions:
		...
		if output_amount not in ALLOWED_OUTPUT_AMOUNTS:
			return _failure("invalid_t03_output_amount:%s" % instance_id)
...
func _normalized_states(value: Variant, instance_id: String) -> Dictionary:
	if not (value is Array or value is PackedStringArray):
		return _failure("invalid_t03_active_primary_states:%s" % instance_id)
	...
	for raw_state: Variant in value:
		...
		if state not in AWAKE_PRIMARY_STATES or seen.has(state):
			return _failure("invalid_t03_primary_state:%s:%s" % [instance_id, state])
		seen[state] = true
		result.append(state)
	result.sort()
...
func _is_active(organism: Dictionary, definition: Dictionary) -> Dictionary:
	...
	if primary_state != "ASLEEP" and primary_state not in AWAKE_PRIMARY_STATES:
		return _failure("invalid_t03_runtime_primary_state:%s:%s" % [instance_id, primary_state])
...
func _occupied_cells(organism: Dictionary, stress_field_by_cell: Dictionary) -> Dictionary:
	var instance_id: String = String(organism.get("instance_id", ""))
	...
	if not (occupied_value is Array or occupied_value is PackedStringArray):
		return _failure("invalid_t03_occupied_cells:%s" % instance_id)
	...
	for raw_cell: Variant in occupied_value:
		...
		if cell_key.is_empty() or seen.has(cell_key):
			return _failure("invalid_t03_occupied_cell:%s" % instance_id)
		if not stress_field_by_cell.has(cell_key):
			return _failure("t03_cell_outside_stress_field:%s:%s" % [instance_id, cell_key])
		seen[cell_key] = true
