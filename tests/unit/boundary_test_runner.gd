...
func _test_content_document() -> void:
	...
	var document: RefCounted = ContentDocumentScript.parse_utf8_json(good_json, &"species")
...
func _test_save_envelope() -> void:
	...
	var envelope: RefCounted = SaveEnvelopeScript.create(&"profile", payload)
	var serialized: String = envelope.serialize()
	var parsed: RefCounted = SaveEnvelopeScript.parse_and_validate(serialized, &"profile")
